...
class Zoo
  ...
  def info_entry_fee(user)
    value = case user.age
    when 0..5
      @entry_fee[:infant]# p "#{user.name}さんの入場料は#{@entry_fee[:infant]}円です。"
    when 6..12
      @entry_fee[:children]# p "#{user.name}さんの入場料は#{@entry_fee[:children]}円です。"
    when 13..64
      @entry_fee[:adult]# p "#{user.name}さんの入場料は#{@entry_fee[:adult]}円です。"
    when 65..120
      @entry_fee[:senior]
    end
      p "#{user.name}さんの入場料は#{value}円です。"
  end
